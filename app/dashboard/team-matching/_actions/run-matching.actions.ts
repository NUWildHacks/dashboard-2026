...
import type { ActionResult, IntakeRecord, TeamMatchingRunStats, TeamMatchingSettings } from "@/types";
...
export const runMatching = async (): Promise<RunMatchingResult> => {
  try {
    ...
    const intakeSnaps = await db.collection(TEAM_MATCHING_INTAKE_COLLECTION).get();
    ...
    const nameMap = new Map(
      userDocs.map((doc) => [doc.id, doc.exists ? `${doc.data()?.first_name ?? ""} ${doc.data()?.last_name ?? ""}`.trim() : "Unknown"])
    );
    ...
    const stats: TeamMatchingRunStats = {
      ...
      required_cluster_count: result.teams.filter((t) =>
        t.members.some((m) =>
          intakes.find((i) => i.user_id === m.user_id)?.required_teammates.length ?? 0 > 0
        )
      ).length,
      invalid_cluster_count: result.warnings.filter((w) => w.type === "oversized_cluster").length,
    };

    // Batch write: run doc + teams + suggestions
    // Firestore batches max 500 ops; for 500 participants we may have ~125 teams + 500 suggestion docs = ~626 ops.
    ...
    const teamDocs: { id: string; data: object }[] = result.teams.map((team) => {
      ...
      return {
        id: teamRef.id,
        data: { ...team, id: teamRef.id, run_id: runId },
      };
    });

    // Build a teamId lookup by member set for suggestions
    ...
    // Resolve team_id in suggestions
    ...
    const allWrites: { ref: FirebaseFirestore.DocumentReference; data: object }[] = [
      ...
      ...suggestionDocs.map(({ userId, data }) => ({
        ref: db.collection(TEAM_MATCHING_SUGGESTIONS_COLLECTION).doc(userId),
        data,
      })),
    ];
    ...
    return { success: true, runId, stats, warningCount: result.warnings.length };
  } catch (error) {
