...
const TeamMatchingPage = async () => {
  ...
  const [entries, runs, settings, configSnap] = await Promise.all([
    getIntakeEntries(),
    getRuns(),
    getSettings(),
  ...
  const resultsReleased = config?.results_released ?? false;

  return (
    <div className="flex-1 flex flex-col gap-6">
      <TeamMatchingAdmin entries={entries} runs={runs} settings={settings} resultsReleased={resultsReleased} />
    </div>
  );
};
