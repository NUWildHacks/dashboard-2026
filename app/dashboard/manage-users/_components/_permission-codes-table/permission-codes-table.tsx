...
import { ConfirmDeletePermissionCodesDialog } from "..";
...
const PermissionCodesTable = ({ permissionCodes }: PermissionCodesTableProps) => {
  const useConfirmDeleteDialogReturn = useConfirmDeleteDialog<PermissionCode>(
    deletePermissionCodes,
    "permission codes"
  );
  const { handleOpenConfirmDeleteDialog } = useConfirmDeleteDialogReturn;

  const { table, selectedPermissionCodeIds, permissionCodesColumns } = usePermissionCodesTable(
    permissionCodes,
  ...
  return (
    <>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="w-full flex flex-col md:flex-row gap-4">
            ...
            {selectedPermissionCodeIds.length > 0 && (
              <Button
                variant="destructive"
                onClick={() => handleOpenConfirmDeleteDialog(selectedPermissionCodeIds)}
      ...
      <ConfirmDeletePermissionCodesDialog {...useConfirmDeleteDialogReturn} />
    </>
  );
};
